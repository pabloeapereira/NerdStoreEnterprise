...
namespace NSE.Bff.Compras.Controllers
{
    ...
    public class PedidoController : MainController
    {
        ...
        public async Task<IActionResult> AdicionarPedido(PedidoDTO pedido)
        {
            ...
            var produtos = await _catalogoService.GetItemsAsync(carrinho.Itens.Select(p => p.ProdutoId));
        ...
        {
            var pedido = await _pedidoService.ObterUltimoPedidoAsync();
            if (pedido is null)
            {
        ...
        private async Task<bool> ValidarCarrinhoProdutos(CarrinhoDTO carrinho, IEnumerable<ItemProdutoDTO> produtos)
        {
            ...
            foreach (var itemCarrinho in carrinho.Itens)
            {
                ...
                if (produtoCatalogo.Valor != itemCarrinho.Valor)
                {
                    ...
                    var responseRemover = await _carrinhoService.RemoverItemCarrinhoAsync(itemCarrinho.ProdutoId);
                    if (ResponseHaveErrors(responseRemover))
                    {
                    ...
                    var responseAdicionar = await _carrinhoService.AdicionarItemCarrinhoAsync(itemCarrinho);

                    if (ResponseHaveErrors(responseAdicionar))
                    {
